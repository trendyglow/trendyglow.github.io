...
<br>
7. EAI 구춛 유형 중 Message Bus 와 Hybrid를 제외한 빈 칸 1~2에 해당하는 나머지 두 가지 유형을 쓰시오.

    |유형|설명|
    |:-----:|-------------------|
    | (1) | - 중간에 미들웨어를 두지 않고 각 어플리케이션 간 직접 연결<br> - 솔루션 구매 없이 통합, 상대적 저렴하게 통합 가능 <br> - 변경, 재사용 어려움|
    | (2) | - 단일 접덤이 허브 시스템을 통해 데이터를 전송하는 중앙 집중적 방식<br> - 모든 데이터 전송 보장, 확장 및 유지 보수 용이<br> - 허브 장애 시 전체 영향|
